...
const requestBatteryOptimizationPermission = async () => {
  ...
  /**
   * Show only once. If you want to show it every day, store a date instead of "true".
   */
      ...
      },
      {
        ...
        onPress: async () => {
          try {
            ...
            console.warn('[Battery] Direct battery optimization request failed:', error);
...
export const debugLocationTrackingStatus = async () => {
  ...
  const message = [
    `GPS Enabled: ${result.gpsEnabled ? 'Yes' : 'No'}`,
    `Foreground Permission: ${result.foregroundPermission}`,
    `Background Permission: ${result.backgroundPermission}`,
    `Expo Background Task Running: ${result.backgroundTaskRunning ? 'Yes' : 'No'}`,
    `Native Service Available: ${result.nativeLocationAvailable ? 'Yes' : 'No'}`,
    `Native Service Started: ${result.nativeBackgroundTrackingStarted ? 'Yes' : 'No'}`,
    `Socket Connected: ${result.socketConnected ? 'Yes' : 'No'}`,
    `Active Session: ${result.activeSessionId || 'Missing'}`,
    `Owner ID: ${result.ownerId || 'Missing'}`,
    `App State: ${result.currentAppState}`,
    `Last Error: ${result.lastUploadError || 'None'}`,
  ].join('\n');

  Alert.alert('Location Debug Status', message);
