...
type EntityRow = {
  uuid: string; server_id: string | null; data: string; version: number;
  sync_status: LocalEntity['syncStatus']; deleted_at: string | null;
...
const addToQueue = async (
  ...
  if (existing) {
    await database.runAsync(
      ...
      finalOperation, JSON.stringify(payload), now, existing.id,
    );
    return;
  }
  await database.runAsync(
    `INSERT INTO sync_queue
     ...
     VALUES (?, ?, ?, ?, ?, ?, 'PENDING', 0, ?, ?)`,
    createUuid(), ownerId, entity, recordId, operation, JSON.stringify(payload), now, now,
  );
};
...
export class BaseRepository<T extends Record<string, unknown>> {
  ...
  async findAll(ownerId: string, options: { search?: string; page?: number; limit?: number } = {}) {
    ...
    const rows = await database.getAllAsync<EntityRow>(
      ...
      ownerId, query ?? '', `%${query ?? ''}%`, limit, (page - 1) * limit,
    );
  ...
  async create(ownerId: string, data: T) {
    ...
    await withTransaction(async (database) => {
      await database.runAsync(
        ...
        uuid, ownerId, JSON.stringify(data), searchText(data), now, now,
      );
  ...
  async update(ownerId: string, id: string, changes: Partial<T>) {
    ...
    await withTransaction(async (database) => {
      await database.runAsync(
        ...
        JSON.stringify(data), searchText(data), now, ownerId, current.uuid,
      );
  ...
  async upsertRemote(ownerId: string, record: DownloadRecord) {
    ...
    await database.runAsync(
      `INSERT OR REPLACE INTO ${this.table}
       ...
       VALUES (?, ?, ?, ?, ?, ?, 'SYNCED', ?, ?, ?)`,
      record.uuid, record.id ?? null, ownerId, JSON.stringify(record.payload), searchText(record.payload),
      record.version, record.deletedAt ?? null, createdAt, record.updatedAt,
    );
  }
