...
// // v9 forces one full download after fixing incremental master-data metadata.
// // Normal server sync still remains incremental: records are sent by updatedAt > lastSync.
// // Pending local operations remain untouched and upload before this download.
// const LAST_SYNC_KEY = 'last_sync_time_v9';
// const BATCH_SIZE = 50;
// const MAX_RETRIES = 8;
...
//     await database.runAsync(
//       `UPDATE sync_queue SET status = 'SYNCING', updated_at = ? WHERE id IN (${ids
//         .map(() => '?')
//         .join(',')})`,
//       new Date().toISOString(),
//       ...ids,
...
//   await database.runAsync(
//     `UPDATE sync_queue SET status = 'PENDING', updated_at = ? WHERE id IN (${items
//       .map(() => '?')
//       .join(',')})`,
//     new Date().toISOString(),
...
// // // Normal server sync still remains incremental: records are sent by updatedAt > lastSync.
// // // Pending local operations remain untouched and upload before this download.
// // const LAST_SYNC_KEY = 'last_sync_time_v10';
// // const BATCH_SIZE = 50;
// // const MAX_RETRIES = 8;
// // const BASE_RETRY_MS = 2_000;
